...
try:
    ...
    print("No file or directory with the name {0}".format(images))
    exit()
...
# pytorch variable for image
im_batches = list(map(prep_image, loaded_ims, [inp_dim for x in range(len(imlist))]))

# list containing the dim of  original images
im_dim_list = [(x.shape[1], x.shape[0]) for x in loaded_ims] # [(W,H),...]
...
if batch_size != 1:
    num_batches = num_images // batch_size + leftover
    im_batches = [torch.cat((im_batches[i*batch_size : min((i + 1)*batch_size, num_images)])) for i in range(num_batches)]
...
# transform the corner attributes of the each bounding box to the original dim of images
